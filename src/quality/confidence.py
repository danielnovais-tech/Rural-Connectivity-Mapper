...
class ConfidenceCalculator:
    ...
    @classmethod
    def _calculate_recency_score(cls, timestamp: datetime, current_time: datetime) -> float:
        ...
        if age_days < 0:
            ...
            score = 100.0 * (1 - (age_days - cls.FRESH_THRESHOLD) / (cls.STALE_THRESHOLD - cls.FRESH_THRESHOLD))
    ...
    def _calculate_consistency_score(cls, measurement: MeasurementSchema) -> float:
        ...
        if measurement.download_mbps is not None:
            if measurement.download_mbps < cls.MIN_DOWNLOAD_MBPS or measurement.download_mbps > cls.MAX_DOWNLOAD_MBPS:
                penalties.append(30.0)  # Major penalty for out-of-range

        # Check upload speed
        if measurement.upload_mbps is not None:
            if measurement.upload_mbps < cls.MIN_UPLOAD_MBPS or measurement.upload_mbps > cls.MAX_UPLOAD_MBPS:
                penalties.append(30.0)

        # Check latency
        if measurement.latency_ms is not None:
            if measurement.latency_ms < cls.MIN_LATENCY_MS or measurement.latency_ms > cls.MAX_LATENCY_MS:
                penalties.append(30.0)
