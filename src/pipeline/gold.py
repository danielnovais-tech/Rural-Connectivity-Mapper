...
class GoldLayer:
    ...
    def _aggregate_geographic(self, measurements: list[MeasurementSchema]) -> Path:
        ...
        def _create_agg_dict() -> dict[str, Any]:
            return {
                "measurements": [],
                "download_sum": 0.0,
                "download_count": 0,
        ...
        h3_aggregates: dict[str, dict[str, Any]] = defaultdict(_create_agg_dict)
        ...
        for h3_index, agg in h3_aggregates.items():
            aggregated[h3_index] = {
                ...
                "avg_download_mbps": round(agg["download_sum"] / agg["download_count"], 2)
                if agg["download_count"] > 0
                else None,
                "avg_upload_mbps": round(agg["upload_sum"] / agg["upload_count"], 2)
                if agg["upload_count"] > 0
                else None,
                "avg_latency_ms": round(agg["latency_sum"] / agg["latency_count"], 2)
                if agg["latency_count"] > 0
                else None,
                "avg_confidence_score": round(agg["confidence_sum"] / agg["confidence_count"], 2)
    ...
    def _aggregate_by_source(self, measurements: list[MeasurementSchema]) -> Path:
        ...
        """
        source_aggregates: dict[str, dict[str, Any]] = defaultdict(
            lambda: {
                "count": 0,
                "confidence_sum": 0.0,
                "measurements": [],
        ...
        for _source, agg in source_aggregates.items():
            agg["avg_confidence"] = round(agg["confidence_sum"] / agg["count"], 2) if agg["count"] > 0 else 0.0
