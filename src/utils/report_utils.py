...
def _generate_txt_report(data: list[dict], output_path: str | None = None, language: str = "en") -> str:
    ...
    try:
        ...
        for i, point in enumerate(data, 1):
            ...
            latitude = point.get("latitude", "N/A")
            longitude = point.get("longitude", "N/A")
            lines.append(f"{get_translation('location', language)}: ({latitude}, {longitude})")
            ...
            if "speed_test" in point:
                ...
                mbps_label = get_translation("mbps", language)
                ms_label = get_translation("ms", language)

                lines.append(f"\n{get_translation('speed_test', language)}:")
                lines.append(f"  {get_translation('download', language)}: {st.get('download', 'N/A')} {mbps_label}")
                lines.append(f"  {get_translation('upload', language)}: {st.get('upload', 'N/A')} {mbps_label}")
                lines.append(f"  {get_translation('latency', language)}: {st.get('latency', 'N/A')} {ms_label}")
