...
def analyze_temporal_evolution(data: list[dict], language: str = "en") -> dict:
    ...
    try:
        ...
        if not data:
            ...
            return {"total_points": 0, "date_range": {}, "daily_averages": {}, "trends": {}, "insights": []}
        ...
        if best_provider:
            insights.append(
                get_translation("insight_best_provider", language, provider=best_provider, score=f"{best_avg:.1f}")
            )

        # Prepare date range
        dates = sorted(daily_data.keys())
        date_range = {"start": dates[0] if dates else None, "end": dates[-1] if dates else None, "days": len(dates)}
        ...
        result = {
            ...
            "insights": insights,
            "provider_stats": {
                provider: {"count": len(scores), "avg_score": round(sum(scores) / len(scores), 2) if scores else 0}
                for provider, scores in provider_stats.items()
            },
        }
...
def cluster_connectivity_points(data: list[dict], n_clusters: int = 3) -> dict:
    ...
    try:
        ...
        if not data or len(data) < n_clusters:
            ...
            return {"clusters": {}, "cluster_labels": [], "cluster_stats": {}, "n_clusters": 0, "features_used": []}
        ...
        for cluster_id, points in clusters.items():
            ...
            cluster_stats[cluster_id] = {
                "count": len(points),
                "centroid": {feature_names[i]: round(centroids[cluster_id][i], 2) for i in range(len(feature_names))},
                "avg_metrics": {
                    feature_names[i]: round(np.mean(cluster_array[:, i]), 2) for i in range(len(feature_names))
                },
                "std_metrics": {
                    feature_names[i]: round(np.std(cluster_array[:, i]), 2) for i in range(len(feature_names))
                },
            }
...
def forecast_quality_scores(data: list[dict], forecast_horizon: int = 5) -> dict:
    ...
    try:
        ...
        if not data:
            ...
            return {"forecasts": [], "baseline_score": 0, "trend": "stable", "confidence": "low"}
        ...
        if len(historical_scores) >= 3:
            ...
            if NUMPY_AVAILABLE and np is not None:
                ...
                older_avg = (
                    float(np.mean(historical_scores[:-3]))
                    if len(historical_scores) > 3
                    else float(historical_scores[0])
                )
            else:
            ...
            try:
                ...
                if cluster_stats:
                    cluster_quality_scores = [
                        stats["centroid"].get("quality_score", baseline_score) for stats in cluster_stats.values()
                    ]
                    if NUMPY_AVAILABLE and np is not None:
                    ...
                    forecasts = [round(0.7 * f + 0.3 * cluster_avg, 2) for f in forecasts]
            except Exception as e:
        ...
        result = {
            ...
            "historical_mean": round(
                float(np.mean(historical_scores)) if (NUMPY_AVAILABLE and np is not None) else _mean(historical_scores),
                2,
            ),
...
def compare_providers(data: list[dict]) -> dict:
    ...
    try:
        ...
        for point in data:
            ...
            provider_metrics[provider]["quality_scores"].append(quality_score.get("overall_score", 0))
            provider_metrics[provider]["download_speeds"].append(speed_test.get("download", 0))
            provider_metrics[provider]["upload_speeds"].append(speed_test.get("upload", 0))
            provider_metrics[provider]["latencies"].append(speed_test.get("latency", 0))
            provider_metrics[provider]["jitters"].append(speed_test.get("jitter", 0))
            provider_metrics[provider]["packet_losses"].append(speed_test.get("packet_loss", 0))
            provider_metrics[provider]["obstructions"].append(speed_test.get("obstruction", 0))
            provider_metrics[provider]["stabilities"].append(speed_test.get("stability", 0))
        ...
        satellite_providers = [
            p for p in providers_summary.keys() if any(sat in p for sat in ["Starlink", "Viasat", "HughesNet"])
        ]
        ...
        best_provider = max(providers_summary.items(), key=lambda x: x[1]["quality_score"]["avg"])
        insights.append(
        ...
        if len(starlink_variants) >= 2:
            starlink_comparison = {p: providers_summary[p] for p in starlink_variants}
            best_starlink = max(starlink_comparison.items(), key=lambda x: x[1]["quality_score"]["avg"])
            insights.append(
        ...
        if satellite_providers:
            terrestrial_providers = [p for p in providers_summary.keys() if p not in satellite_providers]

            if terrestrial_providers:
                sat_avg_latency = calculate_avg([providers_summary[p]["latency"]["avg"] for p in satellite_providers])
                terr_avg_latency = calculate_avg(
                    [providers_summary[p]["latency"]["avg"] for p in terrestrial_providers]
                )
