...
def calculate_distance_from_major_city(latitude: float, longitude: float) -> float:
    ...
    for city_lat, city_lon in major_cities:
        ...
        a = (
            math.sin(dlat / 2) ** 2
            + math.cos(math.radians(latitude)) * math.cos(math.radians(city_lat)) * math.sin(dlon / 2) ** 2
        )
    ...
    try:
        ...
        if len(data) < 3:
            ...
            for point in data:
                ...
                distance = calculate_distance_from_major_city(point.get("latitude", 0), point.get("longitude", 0))
...
def identify_expansion_zones(data: list[dict], n_zones: int = 3) -> dict:
    ...
    try:
        ...
        for zone_id in range(n_zones):
            ...
            avg_quality = np.mean([p.get("quality_score", {}).get("overall_score", 0) for p in zone_points])
            avg_distance = np.mean(
                [calculate_distance_from_major_city(p.get("latitude", 0), p.get("longitude", 0)) for p in zone_points]
            )
        ...
        sorted_zones = dict(sorted(zones.items(), key=lambda x: x[1]["priority_score"], reverse=True))
...
def analyze_starlink_roi(data: list[dict]) -> dict:
    ...
    try:
        ...
        if not data:
            return {"total_points": 0, "rural_points": 0, "high_priority_points": 0, "recommendations": []}
        ...
        avg_current_quality = np.mean([p.get("quality_score", {}).get("overall_score", 0) for p in data])
        ...
        if rural_percentage > 50:
            recommendations.append("STRONG FIT: Over 50% of points are in rural areas - excellent market for Starlink")
        elif rural_percentage > 30:
            recommendations.append("GOOD FIT: Significant rural population - viable Starlink market")

        if avg_current_quality < 60:
            recommendations.append("HIGH OPPORTUNITY: Average quality below 60 - significant room for improvement")
        ...
        if not recommendations:
            recommendations.append("MONITOR: Current connectivity is adequate - focus on maintenance and monitoring")
        ...
        result = {
            ...
            "starlink_suitability_score": round(
                (rural_percentage / 100 * 50) + ((100 - avg_current_quality) / 100 * 50), 2
            ),
        }
...
def generate_ml_report(data: list[dict]) -> dict:
    ...
    try:
        ...
        if not data:
            ...
            empty_report = {
                "summary": {"total_points_analyzed": 0, "ml_model_version": "1.0.0", "analysis_date": "Real-time"},
                "roi_analysis": {},
                "expansion_zones": {},
                "top_priority_areas": [],
                "enriched_data": [],
        ...
        sorted_points = sorted(
            enriched_data, key=lambda x: x.get("ml_analysis", {}).get("priority_score", 0), reverse=True
        )
        ...
        report = {
            "summary": {"total_points_analyzed": len(data), "ml_model_version": "1.0.0", "analysis_date": "Real-time"},
