...
class AnatelSmartConnector:
    ...
    def show_priority_datasets(self, limit: int = 15):
        ...
        for idx, row in high_priority.head(limit).iterrows():
            print(f"\n#{row['Item']} {row['Nome da Base de Dados']}")
            print(f"   Descrição: {row['Descrição da Base de Dados'][:100]}...")
            print(f"   Disponível em: {row['Link dados.gov.br'] if pd.notna(row['Link dados.gov.br']) else 'N/A'}")
    ...
    def generate_download_guide(self):
        ...
        with open(guide_path, 'w', encoding='utf-8') as f:
            ...
            for _, row in high_priority.iterrows():
                ...
                f.write(f"- **Prioridade:** ALTA (Infraestrutura física)\n")
                ...
                f.write(f"- **Instruções:** Acesse o link acima, procure pela opção de download (geralmente CSV ou ZIP)\n")
                f.write(f"- **Salve como:** `{row['Nome da Base de Dados'].replace(' ', '_').replace('/', '_')}_{{DATA}}.csv`\n")
    ...
    def auto_download_dataset(self, dataset_url: str, dataset_name: str) -> Optional[Path]:
        """
