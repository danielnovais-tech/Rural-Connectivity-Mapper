...
def upload_csv_data():
    ...
    uploaded_file = st.file_uploader(
        ...
        type=["csv"],
        help=(
            "Upload a CSV file with columns: latitude, longitude, provider, download, "
            "upload, latency, jitter, packet_loss"
        ),
    )
...
def show_anatel_data(country_code: str):
    ...
    if country_code != "BR":
        st.warning(
            "ANATEL data is only available for Brazil. Showing data for "
            f"{country_code}'s telecom regulator would be displayed here."
        )
        return
