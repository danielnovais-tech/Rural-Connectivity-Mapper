...
def test_add_data_point(client, tmp_path):
    ...
    try:
        ...
        response = client.post("/api/data", data=json.dumps(new_point), content_type="application/json")
...
def test_add_data_point_invalid_coordinates(client):
    ...
    response = client.post("/api/data", data=json.dumps(invalid_point), content_type="application/json")
...
def test_add_data_point_missing_fields(client):
    ...
    response = client.post("/api/data", data=json.dumps(incomplete_point), content_type="application/json")
    assert response.status_code == 400
...
def test_recommendation_endpoint_success(client, tmp_path, monkeypatch):
    ...
    request_data = {"latitude": -15.7801, "longitude": -47.9292, "use_case": "rural_home"}

    response = client.post("/api/v2/recommendation", data=json.dumps(request_data), content_type="application/json")
...
def test_recommendation_endpoint_missing_latitude(client):
    ...
    request_data = {"longitude": -47.9292}
...
def test_recommendation_endpoint_with_session_id(client, tmp_path, monkeypatch):
    ...
    request_data = {"latitude": -23.5505, "longitude": -46.6333}
...
def test_recommendation_endpoint_tracks_analytics(client, tmp_path, monkeypatch):
    ...
    request_data = {"latitude": -15.7801, "longitude": -47.9292}
...
def test_recommendation_endpoint_empty_body(client):
    ...
    response = client.post("/api/v2/recommendation", data="", content_type="application/json")
...
def test_recommendation_endpoint_invalid_json(client):
    ...
    response = client.post("/api/v2/recommendation", data="not valid json", content_type="application/json")
