...
Examples:
  ...
  %(prog)s --list-countries
    ...
    parser.add_argument("--list-countries", action="store_true", help="List all available country codes and exit")

    parser.add_argument(
        "--relatorio", choices=["json", "csv", "txt", "html"], help="Generate report in specified format"
    )

    parser.add_argument("--importar", metavar="CSV", help="Import data from CSV file")

    parser.add_argument("--simulate", action="store_true", help="Simulate router impact on quality scores")

    parser.add_argument("--map", action="store_true", help="Generate interactive Folium map")

    parser.add_argument(
        "--starlink-coverage", action="store_true", help="Add Starlink coverage overlay to the map (requires --map)"
    )

    parser.add_argument(
        "--no-starlink-coverage", action="store_true", help="Disable Starlink coverage overlay on the map"
    )

    parser.add_argument("--analyze", action="store_true", help="Analyze temporal evolution")
    ...
    parser.add_argument(
        ...
        help=(
            "Export data for ecosystem integration (hybrid=Hybrid Architecture Simulator, "
            "agrix=AgriX-Boost, ecosystem=Full bundle)"
        ),
    )
